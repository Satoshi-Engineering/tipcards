<template>
  <div class="relative">
    ...
    <div
      ...
      :class="{
        'border-l-0': !isInFirstColumn,
        'border-t-0': !isInFirstRow,
      }"
    />

    <div v-if="cropMarks && isInFirstRow && isInFirstColumn" class="absolute border-l-[0.5px] opacity-50 h-5 -top-7 start-0" />
    <div v-if="cropMarks && isInFirstRow" class="absolute border-l-[0.5px] opacity-50 h-5 -top-7 end-0" />

    <div v-if="cropMarks && isInLastRow && isInFirstColumn" class="absolute border-l-[0.5px] opacity-50 h-5 -bottom-7 start-0" />
    <div v-if="cropMarks && isInLastRow" class="absolute border-l-[0.5px] opacity-50 h-5 -bottom-7 end-0" />

    <div v-if="cropMarks && isInFirstColumn && isInFirstRow" class="absolute border-t-[0.5px] opacity-50 w-5 top-0 -start-7" />
    <div v-if="cropMarks && isInLastColumn && isInFirstRow" class="absolute border-t-[0.5px] opacity-50 w-5 top-0 -end-7" />
